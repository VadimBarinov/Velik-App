...
#### Для корректной работы функции восстановления пароля по электронной почте на сайте, требуется указать параметры для подключения по SMTP. Параметры указываются в файле:
